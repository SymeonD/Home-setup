...
- `tasks/`: Directory for storing any static files that need to be copied to the target hosts.
  ...
  - [`services.yml`](./tasks/services.yml): Install scripts to manage system services, including backup commands. Deploys `backup_homelab_postgres.sh`, `backup_homelab_restic.sh`, `start-homelab.sh`, `stop-homelab.sh` and `restore-homelab.sh`.
...
- `files/`: Directory for storing any static files that need to be copied to the target hosts.
  ...
  - [`start-homelab.sh`](./files/start-homelab.sh): Script to start all homelab services using Docker Compose.
  - [`stop-homelab.sh`](./files/stop-homelab.sh): Script to stop all homelab services using Docker Compose.
- `templates/`: Directory for storing Jinja2 templates that can be rendered and deployed to the target hosts. Needed when configuration files require ansible variable substitution.
  - [`backup_homelab_restic.sh`](./files/backup_homelab_restic.sh): Script to backup specified directories using Restic. Modify the source directories and repository settings as needed. By default backs up `/srv/data/<service>` to `/srv/backup/restic-repo/` using restic tags `--<service>`.
  - [`restore-homelab.sh`](./files/restore-homelab.sh): Script to restore data from backups for all homelab services. Modify when services are added or removed. By default restores `/srv/backup/postgres/<service>` to `/srv/data/postgres/<service>` and `/srv/backup/restic-repo/` with tag `--<service>` to `/srv/data/<service>`.