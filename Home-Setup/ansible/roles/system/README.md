...
- `templates/`: Directory for storing Jinja2 templates that can be rendered and deployed to the target hosts. Needed when configuration files require ansible variable substitution.
  ...
  - [`restore-homelab.sh`](./templates/restore-homelab.sh): Script to restore data from backups for all homelab services. Modify when services are added or removed. By default restores `/srv/backup/postgres/<service>` to `/srv/data/postgres/<service>` and `/srv/backup/restic-repo/` with tag `--<service>` to `/srv/data/<service>`.